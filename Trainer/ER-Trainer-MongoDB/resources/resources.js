...
export const config = {
  ...
  anytime_finish: false, // Neustart jederzeit möglich
  onchange: async (event) => {
    if (event.event !== "next") return;
    const results = event.instance.getValue();
    const section = results.sections.at(-2);
    section.phrase = results.phrases.at(-2);
  ...
  onfinish: {
    callback: async (results, instance) => {
      const section = results.sections.at(-1);
      section.phrase = results.phrases.at(-1);
      if(section.hasOwnProperty("correct")){
        instance.helper.onFinish(onfinish_settings, section, instance.user);
      }
      instance.start();
    },
  },
  user: ["ccm.instance", "././libs/user/ccm.user.js"],
...
export const de = {
  ...
  finish: "Neustart",
  heading:
    "Bitte wählen Sie den zu der Phrase passenden Beziehungstyp in der Auswahlbox aus!",
...
export const en = {
  ...
  finish: "Restart",
  heading:
    "Please select the relationship type in the selection box that matches the phrase!",
...
export const notations = {
  abrial: {
    ...
    centered: true,
    comment:
      "Die Abrial bzw. (min,max)-Notation gibt für jeden an einer Beziehung beteiligten Entitätentyp an, mit wie vielen Entitäten auf der anderen Seite eine Entität dieses Typs mindestens und höchstens in Beziehung steht.",
  },
  arrow: {
    key: "arrow",
    title: "Pfeilnotation",
