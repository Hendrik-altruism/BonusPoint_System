...
( () => {
    ...
    load: function () {
      ...
      return new Promise( ( resolve, reject ) => {
        ...
        args.forEach( ( resource, i ) => {
          ...
          function loadModule() {
            const [ url, ...keys ] = resource.url.split( '#' );
    ...
    helper: {
      ...
      html: function ( html, ...values ) {
        ...
        if ( element.tagName.startsWith( 'CCM-' ) && !advanced.no_evaluation ) {
          ...
          config.root = element;
          ccm.start( element.tagName === 'CCM-APP' ? element.getAttribute( 'component' ) : element.tagName.substr( 4 ).toLowerCase(), config );
        }
  ...
  async function prepareConfig( config={}, defaults={} ) {
    ...
    result = await ccm.helper.integrate( await ccm.helper.solveDependency( config.key ), result );
  ...
  async function changeVersion( component, config ) {
    ...
    if ( source ) {
      component.ccm = window.ccm[ ( await ccm.helper.loadVersion( source ) ).version() ];
