...
export const config = {
  ...
  "onchange": async (event) => {
    ...
    const result = event.instance.getValue();
    const results = result.results.at(-2);
    results.phrase = result.phrases.at(result.results.length-2);
    if(results.hasOwnProperty("correct")){
  ...
  "onfinish": {
    callback: async (result, instance) => {
      const results = result.results.at(-1);
      results.phrase = result.phrases.at(result.results.length-1);
      if(results.hasOwnProperty("correct")){
        instance.helper.onFinish(onfinish_settings, results, instance.user);
      }
      instance.start();
    },
  },
